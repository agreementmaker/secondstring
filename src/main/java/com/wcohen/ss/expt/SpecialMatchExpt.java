...
public class SpecialMatchExpt {
    ...
    public String toString() {
        return "[SpecialMatchExpt.java: " + fileName + "," + learnerName + "," + blockerName + "]";
    }

    ;

    public StringDistance getLearnedDistance() {
        return learnedDistance;
    }
        ...
        if (similarTokenFile != null) {
            ...
            while ((line = in.readLine()) != null) {
                ...
                for (int i = 1; i < toks.length; i++) {
                    ...
                    if (maxDF <= distAsTFIDF.getDocumentFrequency(toks[i]))
                        maxDF = distAsTFIDF.getDocumentFrequency(toks[i]);
