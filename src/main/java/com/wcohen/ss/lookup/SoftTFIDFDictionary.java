...
public class SoftTFIDFDictionary implements FastLookup {
    ...
    public void freeze() {
        ...
        if (DEBUG)
            System.out.println("computing similar-tokens for " + tokenizer.maxTokenIndex() + " tokens, window=" + windowSize);
    ...
    private void trainDistances() {
        ...
        if (DEBUG)
            System.out.println("training took: " + elapsedSec2 + " (i.e. " + elapsedSec1 + " then " + (elapsedSec2 - elapsedSec1) + ") sec");
    }
    ...
    public int lookup(double minScore, String toFind) {
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            if (tok.getIndex() < maxTFIDFScore.length) {
                ...
                if (DEBUG)
                    System.out.println("just found " + likeTokI.size() + " tokens similar to the novel token " + tok);
                for (Iterator j = likeTokI.iterator(); j.hasNext(); ) {
        ...
        for (Iterator i = usefulTokens.iterator(); i.hasNext(); ) {
            ...
            if (DEBUG)
                System.out.println("after " + tok + " with upper bound " + ub + ": " + candidates.size() + " candidates");
        }
    ...
    private void storeUpperBound(Token tok, Token simTok, List usefulTokens, Map upperBoundOnWeight, double sim) {
        ...
        if (DEBUG)
            System.out.println("upper-bounding tok " + simTok + " sim=" + sim + " to " + tok + " upperBound " + upperBound);
        if (DEBUG)
            System.out.println("upperBound = " + tfidfDistance.getWeight(tok) + "*" + maxTFIDFScore[simTok.getIndex()] + "*" + sim);
    ...
    public String getResult(int i) {
        return ((LookupResult) result.get(i)).found;
    }
    ...
    public Object getValue(int i) {
        return ((LookupResult) result.get(i)).value;
    }
    ...
    public double getScore(int i) {
        return ((LookupResult) result.get(i)).score;
    }
